...
if($_SERVER["REQUEST_METHOD"] == "GET")
{
	...
	if(isset($_GET["id"]))
	{
		...
		{
			echo "<ul>";
			foreach($contacts[$_GET["id"]] as $contactName => $contactCity)
			{
				echo "<li>$contactName : $contactCity</li>";
			}
			echo "</ul>";
			
			echo "<a href='contacts.php'>Back to list</a>";			
		}
		else
			{
				echo "<a href='contacts.php'>Back to list</a>";			
		...
		echo "<ul>";
		foreach($contacts as $studentName => $studentGrades)
		{
			echo "<li><a href='contacts.php?id=$studentName'>{$studentGrades["name"]} - {$studentGrades["city"]},{$studentGrades["state"]}</a></li>";
		}
		echo "</ul>";	
	}
